/*
  game_thread.cpp -- thread functions for playing the game
  Four lines are drawn to form the figure #.
  The quadrants are labeled from 0 to 8 with the upper left
  corner labeled as 0, the next column as 1; next row starts at 3.
*/
#include "SDL/SDL.h"
...
Point draw_centers[9] = { 
    ...
    Point ( x0,  y0+2*dy ), Point(  x0+dx, y0 + 2*dy ), Point ( x0+2*dx, y0 +2*dy ) 
};
...
int game_thread ( void *surface )
{
  ...
  while ( !quit ) {
    if ( mouseq.size() > 0 || numq.size() > 0 ){ //either mouse clicked or num key pressed
